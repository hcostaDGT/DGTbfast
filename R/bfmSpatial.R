...
bfmSpatial <- function(x, dates=NULL, pptype='irregular', start, monend=NULL,
    ...
    # out <- mc.calc(x = x, fun = fun, mc.cores = mc.cores, ...) # original code of DGTbfast (does not work on windows: "mc.cores > 1 not supported on windows")
    out<-calc(x2, fun=fun)
    beginCluster()
    system.time(
    out <- clusterR(x2, calc, args=list(fun))
    )
    endCluster()
